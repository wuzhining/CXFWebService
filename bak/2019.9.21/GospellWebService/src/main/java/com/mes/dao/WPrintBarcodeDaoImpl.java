...
public class WPrintBarcodeDaoImpl implements WPrintBarcodeDao {
	...
	@Override
	public List<TempWPrintBarcode> selectMacModel(String mac) {
		String sql = "SELECT M.mac_code,T.pro_model FROM mes1.r_mes_mo_mac_t M,r_mes_pro_pon_mac_t T WHERE M.mc_cd=T.material_id AND M.mac_code=?";
		RowMapper<TempWPrintBarcode> rowMapper = new BeanPropertyRowMapper<TempWPrintBarcode>(TempWPrintBarcode.class);
		List<TempWPrintBarcode> modelList = jdbcTemplate.query(sql, rowMapper, mac);
		return modelList;
	}
